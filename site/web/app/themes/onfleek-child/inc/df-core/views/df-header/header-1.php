...
<header id="df-header-wrapper" class="df-header df-header-1">
	...
	<nav class="df-top-bar <?php echo ($layout_type=="boxed") ? "boxed no-padding":"";?>">
		...
		<?php DF_Megamenu::df_call_topbar();?>
			...
			if( 'yes' == strtolower($social_icon_topbar) ){
				if ( '' !== $rss ) {?><li><a href="<?php echo esc_url( $rss ); ?>" class="df-top-bar-social"><i class="fa fa-rss"></i></a></li><?php } ?>
	...
	<div class="<?php echo ($layout_type=="boxed") ? "boxed no-padding":"";?> df-logo-wrap">
		<div class="df-logo-inner">
			<a href="<?php printf( home_url() );?>">
				<?php echo DF_Header::df_get_header_logo("1"); ?>
			</a>
		</div>
	</div>
	<nav id="megadropdown" class="<?php echo ($layout_type=="boxed") ? "boxed no-padding":"";?> navbar navbar-default df-navbar" > <!-- role="navigation" -->
		...
		<div id="container-menu" class="no-padding collapse navbar-collapse">
			<div id="wraper-mainmenu" class="<?php echo (is_single()) ? 'single' : '';?> clearfix">
				...
				$boxed_no_padding = ( $content_layout_type == 'full' ) ? 'boxed no-padding' : '';
